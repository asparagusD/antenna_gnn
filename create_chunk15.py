"""Generate chunk15_finetuned_evaluation.ipynb."""
import json
...
# =====================================================================
cells.append(md(
r'''# Chunk 15 — Fine-Tuned Evaluation
...
# =====================================================================
# Cell 1 — Install dependencies (standard header)
# =====================================================================
cells.append(md(
r'''## 1. Environment Setup & Dependencies

Install the required Python packages.'''))

cells.append(code(
r'''# Cell 1 — Install dependencies
!pip install scipy numpy matplotlib torch torchvision \
    torch-geometric tqdm scikit-learn pandas -q'''))

# =====================================================================
# Cell 2 — Clone repo (standard header)
# =====================================================================
cells.append(md(
r'''## 2. Repository Setup

Clone the GitHub repository to the local Colab environment and add `src` to the Python path.'''))

cells.append(code(
r'''# Cell 2 — Clone repo (re-clones every session; pulls latest if already exists)
import os
...
# =====================================================================
# Cell 3 — Drive Mount and Paths (standard header)
# =====================================================================
cells.append(md(
r'''## 3. Drive Mount and Path Configuration

Mount Google Drive to access `RAW_DATA` and store checkpoints and artifacts in `DATA_ROOT`.'''))

cells.append(code(
r'''# Cell 3 — Mount Drive and set data paths
...
cells.append(code(
r'''# CELL A — Setup and normalization guard
import json
import shutil
import torch
...
cells.append(code(
r'''# CELL B — evaluate() with per-sample records, and common_subset_freq_mae
...
cells.append(code(
r'''# CELL C — Train the ceiling model on the FULL pool
...
cells.append(code(
r'''# CELL D — Three models on the held-out test set
...
cells.append(code(
r'''# CELL E — Results table, per grid and pooled
...
# =====================================================================
# CELL F — Ground-truth label reconciliation (regression test)
# =====================================================================
cells.append(md(
r'''---
...
cells.append(code(
r'''# CELL F — Ground-truth label reconciliation on the test set (regression test)
...
cells.append(code(
r'''# CELL G — Selection-effect diagnostic
...
cells.append(code(
r'''# CELL H — Qualitative spectra figure
...
for g in [35, 45, 55]:
    ...
    for cat, pos in found.items():
        ...
        if pos is None:
            print(f'  ⚠ Could not find {cat} sample for grid {g}')
...
cells.append(code(
r'''# CELL I — Data-efficiency curve
...
out_path = 'notebooks/chunk15_finetuned_evaluation.ipynb'
